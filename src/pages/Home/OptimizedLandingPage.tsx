...
export function OptimizedLandingPage({
  ...
  const mockPacks =
    moments?.slice(0, 8).map((moment) => ({
      ...
      title: `${moment.movie.title} Collection`,
      tierLabel: moment.tier.toUpperCase(),
      tagline: `Collect moments from ${moment.movie.title}`,
      currentBid: "Starting at $9.99",
      description: `Iconic moments from ${moment.movie.title}`,
      characterName: moment.characters?.[0]?.name || "Featured Character",
      subtitle: `${moment.movie.release_year}`,
